...
class CPU:
    ...
    def pra(self, op_a, op_b):
        # Print alpha character value stored in the given register
        alpha_value = chr(self.reg[op_a])
        # print(
        #     f'PRN - {alpha_value} is stored in reg[{op_a}]')
        self.pc += 2
        self.prn_list.append(alpha_value)
    ...
    def xor(self, op_a, op_b):
        ...
        # then store the result in registerA.
        # print(
        #     f'XOR - Set reg[{op_a}] to {self.reg[op_a]}+{self.reg[op_b]}')
        self.reg[op_a] = self.reg[op_a] ^ self.reg[op_b]
        self.pc += 3
    ...
    def load(self, code_msg):
        """Load a program into memory."""
        address = 0
        split_lines = code_msg.split('\n')
